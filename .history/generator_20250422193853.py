...
def generate_figure_eight_segment(num_points=100, t_start=0, t_end=2*np.pi):
    """
    Generates points for a segment of a figure-eight knot.
...
# List of available knot generation functions
knot_functions = [generate_trefoil_segment, generate_figure_eight_segment]
...
for i in range(nx):
    for j in range(ny):
        for k in range(nz):
            ...
            # Randomly choose a knot type for this segment
            chosen_knot_func = random.choice(knot_functions)
            base_knot_points = chosen_knot_func(num_points_per_knot, 0, 2*np.pi)
