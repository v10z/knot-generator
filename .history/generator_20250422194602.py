...
for i in range(nx):
    for j in range(ny):
        for k in range(nz):
            ...
            if i + 1 < nx:
                end_node = knot_data[(i + 1, j, k)]['start_point']
                line_points = np.array([start_node, end_node])
                line_poly = pv.lines_from_points(line_points)
                conn_tube = line_poly.tube(radius=connection_tube_radius)
                plotter.add_mesh(conn_tube, color='gray', smooth_shading=True)

            # Connect to neighbor in +j direction
            if j + 1 < ny:
                end_node = knot_data[(i, j + 1, k)]['start_point']
                line_points = np.array([start_node, end_node])
                line_poly = pv.lines_from_points(line_points)
                conn_tube = line_poly.tube(radius=connection_tube_radius)
                plotter.add_mesh(conn_tube, color='gray', smooth_shading=True)
            ...
            if k + 1 < nz:
                end_node = knot_data[(i, j, k + 1)]['start_point']
                line_points = np.array([start_node, end_node])
                line_poly = pv.lines_from_points(line_points)
                conn_tube = line_poly.tube(radius=connection_tube_radius)
                plotter.add_mesh(conn_tube, color='gray', smooth_shading=True)
